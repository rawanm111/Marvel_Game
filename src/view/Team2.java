...
public class Team2 extends JFrame implements ActionListener{
   private Game game;
  ...
  private JLabel PlayerT= new JLabel();
   ...
   Player SecondPlayer;
	...
	public Team2(Player P1,Player p2,Game game) {
	   ...
	   back.add(LokiL);
	    ...
	    Yellow.setName("Yellow Jacket");
	    back.add(YellowL);
	    back.add(Yellow);
	    bCheck( Yellow);
		Player.setText("Choose Leader for Team "+ SecondPlayer.getName());
...
if(e.getSource()==Cap) {
	...
	SecondPlayer.setLeader(s);
	SecondPlayer.getTeam().add(s);
	buttons.add(Cap);
	Cap.setEnabled(false);
		...
		Cap.setEnabled(false);
		 if (buttons.size()==3) {
			    ...
			    this.dispose();
				board board= new board(P1, SecondPlayer);
			}
	...
	Yellow.setEnabled(false);
		...
		public void bCheck( JButton b) {
			ArrayList<Champion> t= P1.getTeam();
			for(Champion c: t) {
